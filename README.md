# ElasticStack
Run syslog-ng, filebeat, logstash, elasticsearch and kibana


ElasticSearch and Kibana:

Go to directory, /Elastic_plus_Kibana/ and run below command to create Kibana and Elasticsearch containers:

sudo /bin/docker compose up -d --build

Run below command to generatge Elasticsearch username and password for Kibana:
//
sudo docker exec -it elasticsearch /usr/share/elasticsearch/bin/elasticsearch-reset-password -u kibana
This tool will reset the password of the [kibana] user to an autogenerated value.
The password will be printed in the console.
Please confirm that you would like to continue [y/N]y


Password for the [kibana] user successfully reset.
New value: xxxyyyzzxyzrandompassword
//


SYSLOG_NG:

1- Run below command which within syslog-ng directory to create docker container:

docker-compose up -d --build

2- Modify syslog-ng.conf file under /syslog-ng/config/ to specify Source and Destination:
## we are saving syslogs to a Log file which will be consumed by filebeat, logstash, elasticsearch and kibana.


